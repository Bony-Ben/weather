...
export default new Vuex.Store({
  ...
  actions: {
    updateWeather({ commit }, loc) {
      axios
        .get(
          `https://api.openweathermap.org/data/2.5/weather?q=${loc}&appid=d9a18e97e763677d7a265e33a139bc2a`
        )
    ...
    addFavorite({ state, commit }, weather) {
      for (let i = 0; i < state.favorites.length; i++)
        if (state.favorites[i].name == weather.name) return;
      commit("addFavorite", weather);
    },
    removeFavorite({ state, commit }, weather) {
      commit(
        "updateFavorites",
        state.favorites.filter((fav) => fav.name != weather.name)
      );
    },
    updateNthFavorite({ commit }, { n, loc }) {
      axios
        ...
        .then((result) =>
          commit("updateNthFavorite", { n: n, weather: result.data })
        );
    },
